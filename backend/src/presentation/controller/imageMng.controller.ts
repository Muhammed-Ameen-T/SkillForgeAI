...
import { normalizeArray } from '../../utils/helpers/arrayNormalize.util';
...
export class ImageController implements IImageController {
   ...
   */
  constructor(
    @inject(TYPES.BulkUploadImagesUseCase) private _bulkUploadUseCase: IBulkUploadImagesUseCase,
    @inject(TYPES.DeleteImageUseCase) private _deleteImageUseCase: IDeleteImageUseCase,
    @inject(TYPES.FindUserImagesUseCase) private _findUserImagesUseCase: IFindUserImagesUseCase,
    @inject(TYPES.EditImageUseCase) private _editImageUseCase: IEditImageUseCase,
    @inject(TYPES.ReorderImageUseCase) private _reorderImageUseCase: IReorderImageUseCase,
  ) {}
  ...
  async bulkUpload(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const titles = normalizeArray<string>(req.body.titles);
      const originalFileNames = normalizeArray<string>(req.body.originalFileNames);
      const mimeTypes = normalizeArray<string>(req.body.mimeTypes);
      const fileSizes = normalizeArray<string>(req.body.fileSizes).map(Number); // Normalize strings, then map to numbers
      ...
      const dto = new BulkUploadDTO(userId, files, titles, originalFileNames, mimeTypes, fileSizes);
